                            ...
                            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                                <div class="p-6">
                                    <button
                                        ...
                                        class="w-full bg-blue-500 hover:bg-blue-700 disabled:bg-gray-400 text-white font-bold py-3 px-4 rounded text-lg transition ease-in-out duration-150"
...
const form = useForm({
    ...
    customer_email: props.sale.customer_email || '',
    items: [],
...
onMounted(() => {
    if (props.sale.items) {
        form.items = props.sale.items.map(item => ({
            ...
            max_quantity: item.product.stock_quantity + item.quantity // Add back current quantity to available stock
        }))
    }
})
