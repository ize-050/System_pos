...
class POSController extends Controller
{
        ...
        try {
            ...
            if ($request->has('product_ids') && !empty($request->product_ids)) {
                ...
                $query->where(function ($q) use ($productIds) {
                    // Include promotions that apply to all products (no specific products/categories)
    ...
    public function applyPromotion(Request $request)
    {
        ...
        $promotion = Promotion::with(['products', 'categories'])->findOrFail($request->promotion_id);
        ...
        foreach ($cartItems as $item) {
            ...
            if ($promotion->products->contains('id', $product->id)) {
                $isEligible = true;
            }
            
            // Check if product's category is included
            if ($promotion->categories->contains('id', $product->category_id)) {
                $isEligible = true;
            }
            
            // If no specific products or categories, apply to all
            if ($promotion->products->isEmpty() && $promotion->categories->isEmpty()) {
                $isEligible = true;
            }
