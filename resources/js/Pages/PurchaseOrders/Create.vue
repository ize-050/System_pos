<template>
  <AppLayout title="สร้างใบสั่งซื้อ">
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">สร้างใบสั่งซื้อ</h2>
    </template>
    <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm rounded-lg p-6">
          <h2 class="text-2xl font-bold mb-6">สร้างใบสั่งซื้อ</h2>
          <form @submit.prevent="submit">
            ...
            <!-- Items -->
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-2">รายการสินค้า *</label>
              <div v-for="(item, index) in form.items" :key="index" class="flex gap-2 mb-2">
                ...
                <input v-model.number="item.quantity_ordered" type="number" min="1" placeholder="จำนวน" class="w-24 rounded-md border-gray-300" />
                <input v-model.number="item.unit_price" type="number" step="0.01" placeholder="ราคา" class="w-32 rounded-md border-gray-300" />
                <span class="py-2">{{ formatCurrency(item.quantity_ordered * item.unit_price) }}</span>
                <button type="button" @click="removeItem(index)" class="text-red-600">ลบ</button>
              </div>
              <button type="button" @click="addItem" class="mt-2 text-indigo-600">+ เพิ่มสินค้า</button>
            </div>
            ...
            <div class="flex justify-end gap-2">
              <Link :href="route('purchase-orders.index')" class="px-4 py-2 bg-gray-300 rounded-md">ยกเลิก</Link>
              <button type="submit" :disabled="form.processing" class="px-4 py-2 bg-indigo-600 text-white rounded-md disabled:opacity-50">สร้างใบสั่งซื้อ</button>
            </div>
          </form>
        </div>
      </div>
...
const form = useForm({
  ...
  items: [{ product_id: '', product_name: '', quantity_ordered: 1, unit_price: 0 }],
  discount_amount: 0,
  include_vat: true,
  notes: '',
});
