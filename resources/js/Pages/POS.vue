          ...
          <div class="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div class="p-6">
              ...
              <div class="mb-6">
                ...
                <div class="relative">
                  <button
                    @click="showPromotionDropdown = !showPromotionDropdown"
                    ...
                    :class="{
                      'border-green-500 bg-green-50': appliedPromotion,
                      'border-gray-300': !appliedPromotion
                    }"
                  >
                    <div class="flex items-center">
                      <div v-if="appliedPromotion" class="flex items-center">
                        <div class="flex-shrink-0 mr-3">
                          <div v-if="appliedPromotion.type === 'percentage'" class="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                            <svg class="w-3 h-3 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                              <path fill-rule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path>
                            </svg>
                          </div>
                          <div v-else-if="appliedPromotion.type === 'fixed_amount'" class="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                          ...
                          <div v-else class="w-6 h-6 bg-purple-100 rounded-full flex items-center justify-center">
                        ...
                        <div>
                          <div class="font-medium text-sm text-gray-900">{{ appliedPromotion.name }}</div>
                          <div class="text-xs font-medium" :class="getPromotionColorClass(appliedPromotion.type)">
                            {{ getPromotionLabel(appliedPromotion) }}
                          </div>
                        </div>
                      </div>
                    ...
                    <svg class="w-5 h-5 text-gray-400 transition-transform" :class="{ 'rotate-180': showPromotionDropdown }" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                    </svg>
                  </button>

                  <!-- Dropdown Menu -->
                  <div v-if="showPromotionDropdown" class="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    <!-- No Promotion Option -->
                    <div
                      @click="removePromotion(); showPromotionDropdown = false"
                      class="px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100"
                      :class="{ 'bg-blue-50': !appliedPromotion }"
                    >
                      <div class="flex items-center">
                        <div class="flex-shrink-0 mr-3">
                        ...
                        <div>
                          <div class="font-medium text-sm text-gray-900">ไม่ใช้โปรโมชั่น</div>
                          <div class="text-xs text-gray-500">ราคาปกติ</div>
                        </div>
                        <div class="flex-shrink-0 ml-auto">
                          <svg v-if="!appliedPromotion" class="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div v-if="availablePromotions.length === 0" class="px-4 py-3 text-center text-gray-500">
                      <div class="flex items-center justify-center">
                        <svg class="w-5 h-5 text-gray-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                          <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>
                        </svg>
                        <span class="text-sm">ไม่มีโปรโมชั่นที่ใช้ได้สำหรับสินค้าในตะกร้า</span>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      :key="promotion.id"
                      @click="applyPromotion(promotion); showPromotionDropdown = false"
                      class="px-4 py-3 hover:bg-gray-50 cursor-pointer"
                      :class="{ 'bg-green-50': appliedPromotion?.id === promotion.id }"
                    >
                      <div class="flex items-center">
                        <div class="flex-shrink-0 mr-3">
                          <div v-if="promotion.type === 'percentage'" class="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                            <svg class="w-3 h-3 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                              <path fill-rule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path>
                            </svg>
                          </div>
                          <div v-else-if="promotion.type === 'fixed_amount'" class="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                            <svg class="w-3 h-3 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                              <path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"></path>
                              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clip-rule="evenodd"></path>
                            </svg>
                        ...
                        <div class="flex-1">
                          ...
                          <div class="text-xs text-gray-500">{{ promotion.description }}</div>
                        ...
                        <div class="flex-shrink-0 ml-2">
                          <svg v-if="appliedPromotion?.id === promotion.id" class="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                          </svg>
                        </div>
                      </div>
                ...
                <div v-if="appliedPromotion" class="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center">
                      <svg class="w-4 h-4 text-green-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                      </svg>
                      <span class="text-sm font-medium text-green-800">โปรโมชั่นที่ใช้: {{ appliedPromotion.name }}</span>
                    </div>
                    <span class="text-sm font-bold text-green-600">-฿{{ formatPrice(promotionDiscount) }}</span>
                  </div>
                </div>
              </div>
